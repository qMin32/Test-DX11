...
//		ResetCapture();
	...
	{
		Tracenf("WindowManager::SetScreenSize(%ld, %ld)", lWidth, lHeight);
		m_lWidth	= lWidth;
		m_lHeight	= lHeight;
	...
	void CWindowManager::Render()
	{
		...
		{
			int totalWindows = 0;
			for (auto* layer : m_LayerWindowList)
			{
				if (layer) totalWindows += (int)layer->GetChildCount();
			}
			Tracenf("WindowManager::Render — %d top-level windows in %d layers", totalWindows, (int)m_LayerWindowList.size());
		}
