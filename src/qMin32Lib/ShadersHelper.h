...
static bool CompileShaderFromFile(const std::string& path, const char* profile, ComPtr<ID3DBlob>& outBlob)
{
	...
	HRESULT hr = D3DCompileFromFile(StringToWString(path).c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", profile, flags, 0, outBlob.GetAddressOf(), errorBlob.GetAddressOf());
...
static bool LoadVertexShader(ID3D11Device* m_device, const std::string& name, const char* profile, ComPtr<ID3D11VertexShader>& vsOut, ComPtr<ID3D11InputLayout>& layoutOut)
{
	if (!m_device)
	{
		return false;
...
#ifdef _DEBUG
	if (!CompileShaderFromFile(path, profile, vsBlob))
		return false;
#else
	...
	hr = m_device->CreateInputLayout(layoutDesc.data(), (UINT)layoutDesc.size(), vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), newLayout.GetAddressOf());
	if (FAILED(hr))
	{
		return false;
	}
...
static bool LoadPixelShader(ID3D11Device* m_device, const std::string& name, const char* profile, ComPtr<ID3D11PixelShader>& psOut)
{
	if (!m_device)
	{
		return false;
	...
	if (!CompileShaderFromFile(path, profile, psBlob))
		return false;
#else
	ComPtr<ID3DBlob> psBlob;
